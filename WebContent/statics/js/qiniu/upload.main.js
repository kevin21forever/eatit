function qiniuUploaderByToken(upkey, button, uptoken, callback) {
	var domain = "https://himg.bonapp.cn/";
	var uploader = Qiniu.uploader({
	    ...
	    init: {
	        ...
	        'Error': function(up, err, errTip) {
	               //上传出错时,处理相关的事情
	        	console.log(JSON.stringify(up) + "---" + JSON.stringify(err));
	        	alert("uploaded failurely");
...
function qiniuUploader(upkey, button, callback) {
	var uptokenUrl = ctx + '/upload/get_uptoken.json';
	var domain = "https://himg.bonapp.cn/";
	var uploader = Qiniu.uploader({
	    runtimes: 'html5,flash,html4',    //上传模式,依次退化
	    browse_button: button,       //上传选择的点选按钮，**必需**
